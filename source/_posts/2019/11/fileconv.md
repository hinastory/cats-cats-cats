...
入力ファイルは`input_dir`と`input_ext`によって選択されます。これによって選択されたファイルは`input_files`フックで上書きでききます。
...
`Line`コンバータはファイルを読み込んで改行コードで区切って`#convert_line`に渡してくれるコンバータです。改行コードは指定がなければ維持されます[^2]。`@opts[:new_line]`を指定することで明示的に改行コードを変換することもできます。
...
[^2]: 最初に読み込んだファイルの最初の改行コードが採用されます。
...
他にも色々失敗した点はありますが、上記の3つは似たような基盤ライブラリを作る上でも参考になると思います。
...
このライブラリの今後についてですが、最近はあまり複雑なコンバータを書くのをやめて単純なコンバータをつなげて処理を書くことが多くなっています。従ってそのうちその知見を生かして`fileconv`にコンバータの「合成」を実装するかもしれません。

本記事が`fileconv`の活用およびファイル一括変換の一助になれば幸いです。