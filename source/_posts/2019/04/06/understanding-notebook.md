...
[^1]: REPLは「Read Eval Print Loop」の略で、プログラムを一行ごとに評価して評価結果を表示するプログラムを意味しています。PythonにおけるiPythonやRubyにおけるirbにあたります。インタラクティブシェルと呼ばれることもあります。

## ノートブックとは

ノートブックとは、簡単に言うとプログラムとその実行結果をまとめたものです。実行結果とはプログラム自体が画面に出力したものと、プログラムの最後の行の評価結果の戻り値になります。「評価結果の戻り値」という表現が分かりづらいかもしれないので、以下のノートブックの一部を使って説明します。

{% img /gallery/daily/tools/jupyter/notebook-1.png  %}

上記の灰色の枠内がプログラムです。半径5の円の面積を計算しています。その枠の下の「`78.53981633974483`」が評価結果の戻り値になります。これはプログラムの最後の行の変数「`math.pi * r ** 2`」の評価結果の戻り値に対応しています。

初めての人が陥りやすい罠として、最後の行を「`area = math.pi * r ** 2`」のような代入文にして、何も表示されず困るというものがあります。これはPythonの代入が戻り値を持たない仕様のためにおこる現象です[^2]。この場合は代入文の後に「`area`」を追加して最終行を表示したい変数に変更するか、以下のようにprint関数で明示的に画面出力すればよいです[^3]。

{% img /gallery/daily/tools/jupyter/notebook-2.png  %}

さらにノートブックにはテキスト以外にも様々なものが表示できます。以下は表を表示してみました。
...
このようにノートブックではプログラムの最後の行の戻り値が自動的に表示される仕組みなので、対話的にプログラムの動作確認したい作業にとても向いています。また、プログラムと実行結果が一緒に保存されるので、プログラムとその動作結果の対応がわかりやすく、後から確認したり復習したりするのも楽なので、プログラミングの学習にも向いています。


[^2]: Pythonでは代入以外にも`if`や`whine`等の制御構造も戻り値を持ちません。Rubyのように代入や制御構造が戻り値を持つ言語もあります。
[^3]: printは戻り値を持たない関数なので、戻り値は表示されません。しかし一般的なPythonの関数は戻り値を持つもの(`return`で値を返す)が多いので、大抵は関数の結果確認のために「最終行」を有効活用できます。
...
従って、ノートブックを先頭から再実行する可能性があり、かつセルを一番下以外に挿入する場合は、セルを一番上から実行しても問題ないかということを意識する必要があります。このトピックはおそらくノートブック利用者が一度は通る道だと思い言及しました。

[^5]: ノートブックの図はJupyter Notebookの新インターフェースであるJupyter Labを元に作成しています。Jupyter Notebookだと若干見え方は異なりますが、ここで説明される概念は共通です。

## ノートブックとマークダウン

これまでセルにはプログラムを記述できると述べましたが、実はマークダウン形式も選択可能です。ノートブックを使わない場合にソースコードにドキュメントを埋め込む場合はコメントを利用することが一般的ですが、コメントはコードに紛れると視認性が悪いという問題点があります。しかし、ノートブックであればコードセルとマークダウンセルが異なるために紛れる心配もなく、マークダウンでリッチなドキュメントを書くことができます。特にマークダウンで見出しだけでも書いておけば、ノートブックのどの位置に目的のコードを書いたのかを見つけるのが容易になります。正直この見出しだけでもマークダウンを使う価値があると思っています[^6]。

[^6]: 更にJupyter Notebookの拡張機能を使えばTOC(目次)機能も使えるようになり、便利さの度合いが跳ね上がります。
...
JupyterLabはJupyter Notebookの次世代ユーザインターフェースです。タブインタフェースで非常に使いやすく自分も愛用しています。まだ、バージョン1.0には達していませんが、普段の利用にはほぼ問題ない感じです。安定性や情報の多さをとるならJupyter Notebookに軍配があがりますが、先進的な機能や格好良さを求めるならJupyterLabをおすすめします。以下の画面は自分が実際に利用しているJupyterLabの画面です[^9]。

{% img /gallery/daily/tools/jupyter/jupyterlab.png  %}

[^9]: 画面に写り込んでいるプログラムは {% elink PythonによるAIプログラミング入門 https://www.oreilly.co.jp/books/9784873118727/ %}の例題になります。

## ノートブックを始めてみる

ノートブックの始め方はいろいろあるので、ざっくりとしたチャートを自分の主観で書いてみました。最初の一歩を踏み出すための道標としてご利用ください[^8]。
...
|Go Back To THIS PAGE|このページの先頭から再度読み直してください|


[^8]: ツッコミどころがあるのは理解しております。スルーして頂けると幸いです。


## ノートブックの用途

ノートブックは非常に用途が広いので、プログラミングにおける様々な場面で活用することができると思います。以下は思いついた用途です。
...
- 「ノートブック」はコードと実行結果をまとめたものである
- 「ノートブック」の実体はファイルである
- 「ノートブック」はマークダウン、HTML、PDF、実行スクリプトなど様々な形式に変換できる
- 「ノートブック」を利用できる複数のクラウドサービスが存在する
- 「ノートブック」にはプログラミング全般に幅広い用途がある

Jupyter Notebookはデータサイエンスの分野ではほぼ必須とまで言われるツールになりましたが、一般のプログラマへの浸透具合はいまいちと感じたのでデータサイエンスの文脈からなるべく切り離して「ノートブック」の説明をしてみました。
