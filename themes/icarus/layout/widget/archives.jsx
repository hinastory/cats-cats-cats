...
Archives.Cacheable = cacheComponent(Archives, 'widget.archives', (props) => {
  ...
  const data = [];
  let length = 0;
  const past_year = ['2018', '2019', '2020', '2021', '2022'];
  ...
  posts.forEach((post) => {
    ...
    const year_type = past_year.some(e => e == year);
    const name = date.format(format || year_type ? 'YYYY' : 'YYYY-MM');
  ...
  const link = (item) => {
    ...
    const year_type = past_year.some(e => e == item.year);
